...
export default function SlotsPage() {
  ...
  const { data: lastSpin } = useReadContract({ address: HOUSE_OF_JOSHI_CONTRACT, abi: houseOfJoshiAbi, functionName: "lastSpin", args: account.address ? [account.address] : undefined, query: { enabled: hasContractAddress && Boolean(account.address) } });
  ...
  return (
    <Shell>
      ...
      <section className="machine"><div>J</div><div>O</div><div>S</div><button type="button" disabled={isPending || !hasContractAddress} onClick={spin}>Spin</button></section>
