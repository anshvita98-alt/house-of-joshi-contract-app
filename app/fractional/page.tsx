...
export default function FractionalPage() {
  ...
  const { data: owner } = useReadContract({ address: HOUSE_OF_JOSHI_CONTRACT, abi: houseOfJoshiAbi, functionName: "ownerOf", args: parsed > BigInt(0) ? [parsed] : undefined, query: { enabled: parsed > BigInt(0), retry: false } });
  const { data: tier } = useReadContract({ address: HOUSE_OF_JOSHI_CONTRACT, abi: houseOfJoshiAbi, functionName: "tokenTier", args: parsed > BigInt(0) ? [parsed] : undefined, query: { enabled: parsed > BigInt(0), retry: false } });
  const { data: fractions } = useReadContract({ address: HOUSE_OF_JOSHI_CONTRACT, abi: houseOfJoshiAbi, functionName: "totalFractions", args: parsed > BigInt(0) ? [parsed] : undefined, query: { enabled: parsed > BigInt(0), retry: false } });
  ...
  return (
    <Shell>
      ...
      <section className="form-panel">
        ...
        <div className="actions"><button type="button" disabled={isPending} onClick={split}>Split NFT</button><span className="status">{status}</span></div>
      </section>
